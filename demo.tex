
%%%% contact via <kisfg@hotmail.com, haikureimu@hnu.edu.cn>
% tikzcd 和 dvipdfmx 冲突的处理方法：https://fohte.hateblo.jp/entry/2017/02/03/130829
% 在documentclass处提供dvipdfmx的选项。
% 必须要包含，否则无法插入 png。
\documentclass[12pt, a4paper, dvipdfmx]{article}

\input{format/package}
\input{format/setup}
\input{format/cover_ref}
\graphicspath{{figures/}}

\addbibresource[location=local]{refer.bib}
\begin{document}
    \begin{CJK*}{UTF8}{song}
        \frontmatter
        %%% 前几页必要内容
        \makethesisheaders
        %%% 目录
        \tableofcontents
        \newpage
        %%% 插图索引 可选项
        % 如果图片存在，则渲染
        \iftotalfigures {
            \addcontentsline{toc}{section}{插图索引}
            \listoffigures
            \clearpage
        }\fi
        %%% 附表索引 可选项
        % 如果表格存在，则渲染
        \iftotaltables {
            \addcontentsline{toc}{section}{附表索引}
            \listoftables
            \clearpage
        }\fi
        %%% 正文
        \mainmatter
        \begin{sloppypar} % 防止文字出界而自动调整的环境。
            \setcounter{page}{1}
            \include{sections/1intro_ref}
            \include{sections/2relatedwork_ref}
            \include{sections/3ourwork_ref}
            \include{sections/4experiment_ref}
            \include{sections/5discussion_ref}
            \include{sections/6conclusion_ref}
            %%% 参考文献
            \newpage\nocite{*}
            \addcontentsline{toc}{section}{参考文献}
            \wuhao{
                \printbibliography[
                    heading=bibliography,
                    title={\centerline{\sanhao\hei{参考文献}}\vspace{-1em}},
                ]
            }
            %%% 致谢和附录
            \xiaosi{\include{sections/acknowledge_ref}}
        \end{sloppypar}
    \end{CJK*}
\end{document}